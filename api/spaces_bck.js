...
spaces.prototype.getAll = function(options) {
    ...
    var fn = function(that, resolve, reject, options) {
        var querystring = "";
        if (options.params) {
            querystring = http.querify(options.params);
        }

        http.requestWithAuth(that.ctx.getEndpoint(options.region) + "/v2/spaces?results-per-page=100&" + querystring, that.ctx.auth.token_type, that.ctx.auth.access_token)
